...
### 5c. Known follow-ups (Create Policy panel / task registry)

- **`TrainingManager.VARIABLE_REGISTRY` only covers pelvis height.** `crouch_depth` (the open-ended reward term `g1_crouch` actually optimizes — see `_reward_crouch_depth` in `legged_robot.py`) has the same "setpoint + reference" shape as `base_height` but is only reachable through the raw, uncurated reward-scale grid. Audit other tasks' reward terms for the same shape and promote them the same way, same label/unit/reference treatment `base_height` already gets.
