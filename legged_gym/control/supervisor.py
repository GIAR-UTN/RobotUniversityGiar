...
class PolicySupervisor:
    ...
    def rename_policy(self, old_name: str, new_name: str) -> None:
        """Renames a loaded policy in place — updates the dict key AND the
        Policy object's own `.name` (status()/policy list reporting reads
        that field, not the dict key, in a couple of spots), and follows
        active_name/pending_name if either currently points at `old_name`
        so an in-flight or already-active switch keeps working under the
        new name. Unlike remove_policy(), renaming the active or
        pending-switch policy is fine — nothing about *which* policy is
        running changes, only its name."""
        if old_name == "damping":
            raise ValueError("'damping' is the safety fallback and can't be renamed")
        if old_name not in self.policies:
            raise KeyError(f"Unknown policy '{old_name}'. Loaded: {list(self.policies)}")
        if new_name in self.policies and new_name != old_name:
            raise ValueError(f"a policy named '{new_name}' already exists")
        policy = self.policies.pop(old_name)
        policy.name = new_name
        self.policies[new_name] = policy
        if self.active_name == old_name:
            self.active_name = new_name
        if self.pending_name == old_name:
