...
def get_args():
    ...
    parser.add_argument('--ball',           action='store_true', default=False, help="spawn a physics-enabled ball prop next to the robot (Genesis only, for now)")
    parser.add_argument('--race',           action='store_true', default=False, help="spawn a race track: a start line at the robot's spawn, a finish line ~10m ahead, and a big blue crash-mat wall right behind it to run into (Genesis/viser only, for now -- sim scenery, not part of training)")
